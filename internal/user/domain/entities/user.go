...
var (
	...
	ErrRoleIsRequired      = errors.New("param: 'role' must be 'admin', 'super' or 'user', please try again")
)
...
func NewUser(id string, email string, password string, firstName string, lastName string, Role Roles) (*User, error) {
	user := &User{
		ID:        ulid.Make().String(),
		Email:     email,
		Password:  password,
